/**
 * 발표용 "화면 크게 보기" — 글씨 크기 / 볼드를 사용자별로 저장하고 앱 전체에 적용한다.
...
const DEFAULT_PREFS = { scale: 1, bold: false };

// 이 이상 키우면 좁은 화면에서 레이아웃이 깨지기 쉬워 여기서 상한을 둔다.
...
export function applyA11y(prefs) {
  ...
  document.body.style.zoom = wide ? String(prefs?.scale ?? 1) : '1';
  document.querySelector('.phone-frame')?.classList.toggle('a11y-bold', !!prefs?.bold);
}
