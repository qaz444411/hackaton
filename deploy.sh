...
#            ./deploy.sh -y              확인 프롬프트 없이 진행 (CI 용)
...
#   ※ DB 볼륨을 지우는 명령(down -v 등)은 넣지 않는다. 실사용자 데이터가 있다.
# ---------------------------------------------------------------
set -euo pipefail
...
# 80 은 https 로 301 리다이렉트되므로 헬스체크는 https 를 직접 본다.
# (http 로 찌르면 301 이 떠서 200 비교가 항상 실패한다)
HEALTH_URL="${DEPLOY_HEALTH_URL:-https://35.86.162.39.nip.io/api/health}"
SITE_URL="${DEPLOY_SITE_URL:-https://35.86.162.39.nip.io/}"

FOLLOW_LOGS=0; NO_CACHE=""; STATUS_ONLY=0; ASSUME_YES=0
while [ $# -gt 0 ]; do
  case "$1" in
...
# 리눅스(Actions 러너 포함)에서는 그냥 ssh 를 쓴다.
SSH_BIN="ssh"
...
say() { printf '\n\033[1;36m▶ %s\033[0m\n' "$1"; }
die() { printf '\n\033[1;31m✗ %s\033[0m\n' "$1" >&2; exit 1; }
# 리다이렉트를 따라가서 최종 상태 코드를 본다
http_code() { curl -sL -m "${2:-10}" -o /dev/null -w '%{http_code}' "$1" 2>/dev/null || echo 000; }
...
if [ "$STATUS_ONLY" = 1 ]; then
  ...
  say "외부 응답"
  printf '  %s -> HTTP %s\n' "$SITE_URL"   "$(http_code "$SITE_URL")"
  printf '  %s -> HTTP %s\n' "$HEALTH_URL" "$(http_code "$HEALTH_URL")"
  exit 0
fi

# --- 1. 로컬이 푸시됐는지 확인 ---------------------------------
# 서버는 GitHub 에서 받는다. 안 밀어둔 커밋은 절대 배포되지 않으므로 미리 경고한다.
# CI 에서는 이미 푸시된 커밋으로 도는 것이라 -y 로 건너뛴다.
say "로컬 상태 확인"
if [ "$ASSUME_YES" = 1 ]; then
  echo "  -y 지정됨 — 로컬 상태 확인을 건너뛴다"
else
...
for _ in $(seq 1 30); do
  if [ "$(http_code "$HEALTH_URL" 5)" = "200" ]; then
    printf '  API  %s -> 200\n' "$HEALTH_URL"
    printf '  WEB  %s -> %s\n' "$SITE_URL" "$(http_code "$SITE_URL")"
...
printf '\n\033[1;31m✗ 헬스체크 실패 (%s). 로그를 확인한다:\033[0m\n' "$HEALTH_URL" >&2
