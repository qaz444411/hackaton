...
- **`db:status` — v1~v6 패치가 전부 적용됐는가**

마지막 항목이 중요하다. `git fetch` 가 조용히 실패해도 사이트는 **예전 코드로 계속 200** 을
주기 때문에, 헬스체크만으로는 "배포가 안 됐는데 성공"을 잡아낼 수 없다.


`db:status` 도 같은 이유다. **자동 배포는 코드만 갱신하고 스키마는 건드리지 않는다.**
마이그레이션이 밀린 채 코드만 나가면 사이트는 200 이지만 해당 기능만 500 을 뱉는다.
실제로 v5 누락으로 채팅 목록이 죽었을 때 워크플로우는 초록불이었다.

빠진 패치가 있으면 배포가 **빨간불로 실패**하고, Actions 로그에 이렇게 뜬다.

```
================================================================
 DB 마이그레이션이 밀려 있다 — 코드는 배포됐지만 스키마가 예전이다
================================================================

[빠진 패치]
│ 4 │ '05_patch_v5' │ 'v5 채팅 개선' │ '빠짐' │ '없음: 테이블 chat_room_hidden, ...' │

[돌려야 할 명령] — ssh 로 접속해서 실행한다
  ssh -i <pem> ubuntu@<EC2_HOST>
  cd ~/bapfriend
  docker compose --env-file .env.production exec api \
    node scripts/run-sql.js ../db/05_schema_patch_v5.sql
```

상단 annotation 에 빠진 패치 이름이 요약되고, Job Summary 에 `db:status` 전체 출력이 남는다.

> **배포가 이 단계에서 실패해도 코드는 이미 서버에 올라가 있다.** 롤백이 아니라
> "스키마를 맞춰라"는 신호다. 위 명령을 돌린 뒤 Actions 에서 **Re-run** 하면 초록불이 된다.
### 10-4. 수동 배포
